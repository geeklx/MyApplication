...
public class AnimRunUtil {
    ...
    private static AnimRunUtil floatAnimRunUtil = new AnimRunUtil();

    public static AnimRunUtil getFloatAnimRunUtil() {
        return floatAnimRunUtil;
    }
...
//			Drawable drawable=res.getDrawable(id);
    ...
    public synchronized static int[] generateDrawableArrayStartOne(Activity context, String name, int start, int end) {
...
//			Drawable drawable=res.getDrawable(id);
    ...
    public synchronized static int[] generateDrawableArrayStartOneForth(Activity context, String name, int start, int end) {
        int[] drawaleArr = new int[end - start];
        Resources res = context.getResources();
        for (int i = start; i < end; i++) {
            //name:图片的名，defType：资源类型（drawable，string。。。），defPackage:工程的包名
