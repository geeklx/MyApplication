 ...
 */
package com.example.p022_hois.hois2;
...
public class HiosHelper2 {
    ...
    public static boolean checkUriHost(Uri uri) {
        return UriHelper2.HIOS_SCHEME.equalsIgnoreCase(uri.getScheme());
    }
    ...
    private static void invokeMethod(Activity activity, final Object receiver, Uri uri,
        ...
        List<String> conditions = UriHelper2.queryStringFillMap(map, uri);

        if (!conditions.isEmpty()) {
            if (conditions.contains(UriHelper2.CONDITION_LOGIN)) {
...
//                return;
            ...
            if (conditions.contains(UriHelper2.CONDITION_OR_LOGIN)) {
...
//                }
    ...
    private static void activity(final Activity activity, Uri uri, final Intent intent) {
        ...
        List<String> conditions = UriHelper2.queryStringFillIntent(intent, uri);
        if (!conditions.isEmpty()) {
            if (conditions.contains(UriHelper2.CONDITION_LOGIN)) {
//                UserUtils.get().loginToDo(activity, new Runnable() {
//                    @Override
