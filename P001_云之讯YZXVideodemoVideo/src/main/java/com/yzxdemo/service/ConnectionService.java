...
public class ConnectionService extends Service implements ConnectionListener,
	...
	private BroadcastReceiver br = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals(UIDfineAction.ACTION_LOGIN)) {
				...
				if (intent.hasExtra("cliend_id")
						&& intent.hasExtra("cliend_pwd")) {
					...
					CustomLog.i(DfineAction.TAG_TCP, "CLIENT_ID:" + cliend_id
							+ "   CLIENT_PWD:" + cliend_pwd);
					if (cliend_id != null && cliend_id.length() > 0
							&& cliend_pwd != null && cliend_pwd.length() > 0) {
						if (DataTools.istest) {
							// 公司测试专用
							connectionService("http://113.31.89.144", "8887",
									sid, sid_pwd, cliend_id, cliend_pwd);
						} else {
							// 开发者使用
							connectionService(null, null, sid, sid_pwd,
									cliend_id, cliend_pwd);
						}
					}
				...
				if (intent.hasExtra(UIDfineAction.FROM_NUM_KEY)) {
					fromSerNum = intent
							.getStringExtra(UIDfineAction.FROM_NUM_KEY);
				}
				...
				// UCSService.uninit(ConnectionService.this);
					...
					.equals(UCSService.ACTION_INIT_SUCCESS)) {
				CustomLog.i(DfineAction.TAG_TCP, "启动成功  ... ");
				UCSService.openSdkLog(true);// 输出sdk日志到sd卡中
											// yunzhixun/log/YZX_SDK_日期.txt
			} else if (intent.getAction().equals(
					UIDfineAction.ACTION_START_TIME)) {
				startCallTimer();
			}
		}
	};
	...
	public void onConnectionFailed(UcsReason reason) {
		CustomLog.i(DfineAction.TAG_TCP,
				"CONNECTION_FAILED:" + reason.getReason());
		if (reason.getMsg().length() > 0) {
			CustomLog.i(DfineAction.TAG_TCP,
					"CONNECTION_FAILED:" + reason.getMsg());
		}
		sendBroadcast(new Intent(UIDfineAction.ACTION_TCP_LOGIN_CLIENT_RESPONSE)
				.putExtra(UIDfineAction.RESULT_KEY, 1).putExtra(
						UIDfineAction.REASON_KEY, reason.getReason()));
		if (reason.getReason() == 300505 || reason.getReason() == 300207) {
			sendBroadcast(new Intent(UIDfineAction.ACTION_LOGOUT).putExtra(
					UIDfineAction.REASON_KEY, reason.getReason()));
			Intent intent = new Intent();
			intent.setClass(ConnectionService.this,
					TerminalLoginDialogActivity.class);
			intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra("reason",
					reason.getReason());
	...
	public void onConnectionSuccessful() {
		...
		sendBroadcast(new Intent(UIDfineAction.ACTION_TCP_LOGIN_CLIENT_RESPONSE)
				.putExtra(UIDfineAction.RESULT_KEY, 0));
	...
	public void connectionService(final String host, final String port,
			final String sid, final String sidPwd, final String cliend_id,
			final String cliend_pwd) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				if (host != null && host.length() > 0) {
					UCSService.connect(host, port, sid, sidPwd, cliend_id,
							cliend_pwd);
				} else {
	...
	public void startCallTimer() {
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			...
			public void run() {
				...
				sendBroadcast(new Intent(UIDfineAction.ACTION_CALL_TIME)
						.putExtra("callduration",
								hour * 3600 + minute * 60 + second).putExtra(
	...
	public void onDialFailed(String arg0, UcsReason reason) {
		...
		CustomLog.i("onDialFailed CURRENT_ID:" + arg0 + "          SERVICE:"
				+ reason.getReason() + "   MSG:" + reason.getMsg());
		voipSwitch(reason);
	}
	...
	private void voipSwitch(UcsReason reason) {
		...
		default:
			if (reason.getReason() >= 10000 && reason.getReason() <= 20000) {// 透传错误码
				CustomLog.i(DfineAction.TAG_TCP,
						"KC_REASON:" + reason.getReason());
			} else if (reason.getReason() >= 300233
					&& reason.getReason() <= 300243) {// 回拨
				sendBroadcast(new Intent(UIDfineAction.ACTION_DIAL_STATE)
						.putExtra("state", reason.getReason()));
			}
			break;
		}
	}
	...
	public void onHangUp(String arg0, UcsReason reason) {
		...
		sendBroadcast(new Intent(UIDfineAction.ACTION_DIAL_HANGUP).putExtra(
				"state", reason.getReason()));
		CustomLog.i("onHangUp CURRENT_ID:" + arg0 + "SERVICE:"
				+ reason.getReason());
		voipSwitch(reason);
	}

	/**
	 * 接收新消息 nickName 显示主叫昵称 userdata 暂时没用到
	 */
	@Override
	public void onIncomingCall(String callId, String callType,
		...
		intent.putExtra("phoneNumber", callerNumber).setFlags(
	...
	/**
	 * 视频通话质量回调 reason: 1、质量好，0、质量一般，2、质量差
	 */
	@Override
	public void onNetWorkState(int reason) {
		sendBroadcast(new Intent(UIDfineAction.ACTION_NETWORK_STATE).putExtra(
				"state", reason));
	}
	...
	public void onReceiveUcsMessage(UcsReason reason, UcsMessage message) {
		if (reason.getReason() == 0) {
			ArrayList<UcsMessage> arrayList = IMMessageActivity.msgList
					.get(message.getFormuid());
			if (arrayList == null) {
	...
	public void onSendUcsMessage(UcsReason reason, UcsMessage message) {
		if (message != null) {
			...
			if (reason != null) {
				...
				sendBroadcast(new Intent(UIDfineAction.ACTION_MSG).putExtra(
						UIDfineAction.REASON_KEY, reason.getReason()));
			}
		}
	}
	...
	public void onSendFileProgress(int progress) {
		...
		sendBroadcast(new Intent(UIDfineAction.ACTION_SEND_FILE_PROGRESS)
				.putExtra(UIDfineAction.RESULT_KEY, progress));
	}
	...
	public void onDownloadAttachedProgress(String msgId, String filePaht,
		...
		CustomLog.d(DfineAction.TAG_TCP, "下载文件进度:" + sizeProgrss + "    "
				+ currentProgress);
